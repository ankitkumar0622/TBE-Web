...
const ReactQuestions = ({ seoMeta }: PageProps) => {
  ...
  const fetchQuestions = async () => {
    try {
      const response = await axios.get(`${routes.interviewPrepGetAll}?type=React`);
  ...
  const handleCreateQuestion = async () => {
    ...
    try {
      ...
      setNewQuestion({ name: '', answer: '', type: 'React' });
      setError(null);
    } catch (error) {
  ...
  return {
    props: {
      seoMeta: {
        ...
        description: 'A collection of React interview questions to help you prepare.',
        url: routes.interviewPrepReact,
        robots: 'follow, index',
      },
    },
  };
